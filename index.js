...
const processStock = async (productId, variantName, qtyNeeded, forceHunterMode = false) => {
    ...
    return await db.runTransaction(async (t) => {
        ...
        // 🔥 FITUR BARU: MODE PAKSA HUNTER
        // Jika Admin tekan tombol Paksa Kirim, kita pura-pura stok manual kosong
        // Agar sistem langsung mengambil dari Backup/Sheet
        if (forceHunterMode) {
             return { 
                success: false, 
                currentStock: 0, 
...
const processOrderLogic = async (orderId, orderData, forceHunter = false) => {
    ...
    for (let i = 0; i < orderData.items.length; i++) {
        ...
        try {
            ...
            if (result && result.success) {
                // KASUS A: STOK MANUAL CUKUP (Atau Placeholder Permanen "Menunggu Bayar")
                ...
                // KASUS B: STOK MANUAL KOSONG -> AKTIFKAN "AUTO HUNTER"
                let stockFromDB = [];
                ...
                if (result.backupConfig && stillNeed > 0) {
                    ...
                    if(url && sku) {
                        ...
                        for(let k=0; k<stillNeed; k++) {
                            ...
                            if(hasil.sukses) hunterContent.push(`${hasil.sn}`);
    ...
    // 🔥 PERBAIKAN: PAKSA STATUS 'success' AGAR WEB MENAMPILKAN DATA (AUTO ACC)
...
// FUNGSI SAKTI: FORCE FULFILLMENT (PAKSA KIRIM)
...
const mainMenu = Markup.inlineKeyboard([
    ...
    [Markup.button.callback('📄 ISI STOK (UPLOAD)', 'restock_sheet_ask')], // TOMBOL BARU
...
bot.on(['text', 'photo', 'document'], async (ctx, next) => {
    ...
    let text = "";
    // LOGIKA IMPORT DB & RESTOCK SHEET
    ...
    if (textLower === 'help' || textLower === 'bantuan') {
        return ctx.reply("Panduan Admin...", {parse_mode: 'Markdown'});
    }
    ...
    if (session) {
        ...
        if (session.type === 'ADD_VAR_EXISTING') {
            ...
            if (session.step === 'NAME') { session.tempVar = { name: text, apiList: [] }; session.step = 'CODE'; ctx.reply("Kode Variasi:", cancelBtn); }
                ...
                if (text.toLowerCase() === 'ya') { session.step = 'INPUT_API'; ctx.reply("Format: `URL|KODE|MODAL`", cancelBtn); } else { session.step = 'CONTENT'; ctx.reply("Stok Manual:", cancelBtn); }
            }
        ...
        else if (session.type === 'ASK_SHEET_URL') {
            ...
            ctx.reply("📂 Oke URL disimpan.\nSekarang **Kirim File .txt** berisi daftar stok (Format: KODE|DATA).", cancelBtn);
            return;
        }
...
bot.action(/^back_prod_(.+)$/, async (ctx) => { const d = await db.collection('products').doc(ctx.match[1]).get(); ctx.editMessageText(`🔎 *${d.data().name}*`, Markup.inlineKeyboard([[Markup.button.callback('✏️ Edit Utama', `menu_edit_main_${d.id}`)],[Markup.button.callback('🔀 ATUR VARIASI', `menu_vars_${d.id}`)],[Markup.button.callback('🗑️ Hapus PRODUK', `del_prod_${d.id}`)]])); });
...
// 🔥 TOMBOL SAKTI: PAKSA KIRIM (FORCE FULFILLMENT)
